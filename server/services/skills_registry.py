        ...
        _component(
            ...
            operations=[
                _op(
                    ...
                    invoke="Copilot 经 executor.mobile_actions._run_mobile_click",
        ...
        _component(
            ...
            operations=[
                _op(
                    ...
                    invoke="executor.mobile_actions._run_mobile_input（Plan kind=input）",
                ...
                _op(
                    "mobile_back",
                    "返回键",
                    "注入 Android BACK，按步骤立即执行。",
                    params=[],
                    invoke="executor.mobile_actions._run_mobile_back（Plan kind=back）",
                ...
                _op(
                    "device_prepare",
                    "设备准备 / 唤醒解锁",
                    "执行器前置准备能力：确保设备在线、屏幕可用，必要时唤醒或解锁。该能力不允许由大模型 Plan 静默下发，只能作为执行前置事件记录到执行卡片/报告中。",
                    params=[],
                    invoke="device_bootstrap.ensure_adb_device_online（执行器前置事件）",
                    triggers=["执行前设备未就绪", "锁屏/黑屏/连接恢复"],
                    examples=["设备准备事件：唤醒/解锁完成"],
                    platforms=["android"],
                    risk="可能涉及锁屏密码、设备状态变更，必须在报告中显式记录。",
                ),
                _op(
                    "shell_pm_clear",
                    "清理应用数据",
    ...
    return [
        {
            ...
            "items": [
                ...
                },
                {
                    ...
                    "invoke": "executor.execute_steps.execute_steps(steps, sn, …)",
