...
from server.services.runtime.playwright_hub import WEB_SLOT_SN, get_hub, pick_goto_url
...
class PlaywrightExecutor:
    ...
    def execute(self, event: PlanEvent, ctx: ExecutorContext) -> EventResult:
        ...
        sn = str(ctx.run_context.sn or WEB_SLOT_SN)
        hub = get_hub()
        try:
            if cap == "wait_ms":
                p = event.params or {}
            ...
            if cap == "launch_app":
                ...
                url = pick_goto_url(
                    p.get("url"),
                    ctx.run_context.target_package,
                    p.get("package"),
                )
            ...
            if page is None:
                page = hub.open_case(
                    sn,
                    base_url=pick_goto_url(ctx.run_context.target_package),
                )
            if cap == "tap_element":
