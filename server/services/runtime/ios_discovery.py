# !/usr/bin/env python
# -*-coding:utf-8 -*-
"""iOS 设备发现器：USB(usbmuxd) + 局域网(Bonjour apple-mobdev)，对齐 adb_discovery。

不收录 CoreDevice `_remotepairing`（模拟器 UUID）。
"""
...
_last_seen: Dict[str, str] = {}  # udid -> usb|wifi
...
def _run_tick() -> bool:
    ...
    for item in list_usb_ios_devices():
        ...
        if not udid or is_rfc4122_uuid(udid):
            continue
        present[udid] = {
            "transport": "usb",
    ...
    for udid, item in snapshot_wifi_devices().items():
        if not is_executable_ios_sn(udid):
            continue
        if udid in present:
...
async def run_ios_discovery(interval_sec: float = DISCOVERY_INTERVAL_SEC) -> None:
    ...
    SLog.i(TAG, "Starting iOS discovery (usbmuxd + Bonjour)...")
